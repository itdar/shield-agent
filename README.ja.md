...
shield-agentはリクエストごとにMCP、A2A、HTTP APIのプロトコルを自動的に検出します。設定ファイルで `protocol: auto` を指定するか、upstreamごとに明示的に指定することもできます。

### 検出シグナル

| プロトコル | 検出条件 |
|-----------|----------|
| MCP | `Mcp-Session-Id` ヘッダーあり、またはJSON-RPCかつA2A以外のメソッド |
| A2A | `X-A2A-Signature` ヘッダーあり、またはJSON-RPCかつ `tasks/*` メソッド |
| HTTP API | JSON-RPC構造なし |
...
`protocol: auto`（デフォルト）の場合、shield-agentは各リクエストのヘッダーとペイロード構造を検査してプロトコルを判定し、適切なミドルウェアチェーンを適用します。
...
shield-agentはエージェントのリクエスト履歴をもとにトラストスコアを算出し、スコアに応じてレート制限を自動調整します。

### 動作フロー

```
アクションログ → スコア計算 → トラストレベル → レート制限
```
...
reputation:
  enabled: true
  window: 24h          # スコア計算の対象期間
  min_requests: 10     # スコア計算に必要な最低リクエスト数
```

### CLIコマンド

```bash
shield-agent reputation                  # 全エージェントのトラストスコア一覧
shield-agent reputation <hash>           # 特定エージェントの詳細スコア
```
...
| メソッド | パス | 説明 |
|--------|------|------|
| GET | `/api/reputation` | 全エージェントのスコア一覧 |
| GET | `/api/reputation/{hash}` | 特定エージェントのスコア詳細 |
| POST | `/api/reputation/{hash}/reset` | スコアのリセット |
| GET | `/api/reputation/config` | 評判システムの設定確認 |
...
|------|------|------|
| 成功率 | +0.35 | リクエストの成功割合 |
| エラーペナルティ | -0.25 | エラーレスポンスによる減点 |
| レート超過 | -0.20 | レート制限超過による減点 |
| 利用継続性 | +0.15 | 継続的な利用実績 |
| 異常検知 | -0.05 | 異常なパターン検出時の減点 |

スコアは0.0〜1.0の範囲で計算され、しきい値を下回ったエージェントには自動的に厳しいレート制限が適用されます。
...
| Phase 4 — 高度化 | **一部完了** | エージェント評判 ✅、プロトコル自動検出 ✅、WebSocket（予定） |
