...
```
 INGRESS 모드 — 내 MCP / A2A / API 서버 보호
 ──────────────────────────────────────────────────────────
  외부 Agent           ┌──────────────────┐             내
  /Client ── req  ──▶  │   shield-agent   │  ── req ──▶ MCP /
          ◀── res  ──  │   :8888 proxy    │  ◀── res ── A2A /
                       │ auth·guard·log   │             API
                       └──────────────────┘

 EGRESS 모드 — 외부 AI API 호출 감사
 ──────────────────────────────────────────────────────────
  내 AI                ┌──────────────────┐             OpenAI /
  Agent   ── HTTPS ──▶ │   shield-agent   │  ─ HTTPS ─▶ Anthropic /
          ◀──────────  │   :8889 egress   │  ◀───────── Google /
  (HTTPS_PROXY)        │ MITM·log·policy  │             내부 LLM
                       └──────────────────┘
...
- [Egress 모드 (AI 컴플라이언스)](#egress-모드-ai-컴플라이언스)
...
| **Agent → 외부 AI API** | **HTTPS CONNECT / HTTP** | **egress (AI 컴플라이언스)** |
...
> 에이전트가 **외부 AI API로 나가는 요청**을 가로채어 기록/검증하고 싶을 때

shield-agent를 역방향으로도 쓸 수 있습니다. 기존 proxy/stdio 모드는 에이전트가
받는 요청을 방어하는 반면, egress 모드는 에이전트가 OpenAI / Anthropic / Google 등
외부 AI 서비스로 보내는 **outbound 트래픽**을 가로채어 감사 로그를 남깁니다. 한국
AI기본법(2026-01-22 시행) Art. 23·27·34·35, EU AI Act Art. 12·13·26·50의 감사 로그
보존과 투명성 의무에 맞춰 설계되었습니다.
...
└──────────┘          └─────────┬──────────────┘          └──────────────────┘
                                │
                       egress_logs + 해시 체인 + daily digest
```
...
| 단계 | 모드 | 수집 데이터 | CA 인증서 |
|------|------|-------------|----------|
| **Phase 1** | Metadata-only | 목적지, 타이밍, 크기, provider, 정책 위반 | 불필요 |
| **Phase 2** | per-host TLS MITM | 위 + model, prompt_hash, content_class, PII flags | 필요 (`shield-agent ca init`) |

Phase 1만으로도 "어떤 AI 서비스에 얼마나 요청했는지"가 감사 가능합니다. 본문
수준 추적이 필요할 때만 Phase 2를 호스트별로 opt-in하세요.

### 빠른 시작 — Phase 1 (TLS 복호화 없음)

```bash
# 독립 프로세스로 실행
shield-agent egress --listen 127.0.0.1:8889

# 또는 기존 ingress proxy와 함께
...
요청이 들어오면 `egress_logs`에 row 1개가 기록됩니다 — `provider`, `destination`,
`timing`, `policy_action`, `row_hash`(해시 체인 링크).
...
감사 번들에는 로그 row + 앵커 + 해시 체인 증명 + 정책 스냅샷이 포함되어
규제 기관이 별도 도구로 재검증할 수 있습니다.

### Phase 2 활성화 (TLS MITM)

```bash
# 1) CA 생성
shield-agent ca init
# 2) OS 신뢰 저장소 등록 (macOS keychain / Linux update-ca-certificates)
shield-agent ca trust
```

`shield-agent.yaml` 예시:
```yaml
egress:
  enabled: true
  listen: "127.0.0.1:8889"
  policy_mode: "warn"      # block으로 바꾸면 정책 위반 시 HTTP 403 반환

  # per-host opt-in — 여기 있는 호스트만 TLS MITM됨
  mitm_hosts:
    - "api.openai.com"
    - "api.anthropic.com"
  ...
  hash_chain:
    enabled: true
    algorithm: "sha256"
    # 옵션: DB 외부의 append-only 파일에 매일 체인 tail 기록 (변조 방어)
...
- **Drop-free 로그**: 백프레셔로 감사 로그 유실 방지 (Principle: Auditability > Performance)
- **Fail-closed**: `policy_mode: block` 에서 DB write 실패 시 요청 자체를 HTTP 503으로 거부
- **SSRF 가드**: CONNECT를 통해 RFC1918/loopback/link-local 주소로 나가는 요청 자동 차단
- **해시 체인 + 앵커 purge**: 오래된 로그를 지울 때도 해시 체인 연속성 유지, 변조 시 `--verify`로 탐지
- **외부 daily digest**: DB full-access 공격자를 위한 방어 레이어
- **한국 특화 PII**: 주민등록번호, 한국 전화번호, 운전면허번호 정규식 기본 탑재

### 규제 의무 매핑

| 요건 | shield-agent 기능 |
|------|-------------------|
| EU AI Act Art. 12 (자동 이벤트 기록) | `egress_logs` + 해시 체인 |
| EU AI Act Art. 12(2) (추적 가능성) | `correlation_id` (ingress↔egress) |
| EU AI Act Art. 50 (생성형 AI 표시) | Phase 2: `ai_generated_tag`, `content_class`, `X-AI-Generated` 헤더 주입 |
| EU AI Act Art. 13·26 (투명성·모니터링) | Web UI, Prometheus 메트릭 |
| 한국 AI기본법 제23·27·34·35조 | 감사 로그 보존, 영향평가용 export, 생성형 AI 플래그 |

### 범위 외 (명시)

- 모델 내부 로깅, 학습 데이터 출처 증명 — AI 서비스 제공자의 의무
- 실시간 환각 탐지 — 전문 도구 영역
- 법적 자문 — Phase 2 프로덕션 배포 전 법무 검토 필수 (TLS MITM의 PIPA 제23조 / GDPR Art. 9 리스크)
