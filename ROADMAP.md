...
- [x] go.mod module path → `github.com/itdar/shield-agent`
- [x] 디렉토리 구조 정리
  ```
  cmd/shield-agent/     CLI 엔트리포인트
  internal/
    transport/          stdio, proxy (SSE, streamable-http)
    middleware/         middleware 인터페이스 + 체인 + registry
    middleware/guard/   rate-limit, size-limit, IP block
    middleware/httpauth/ A2A/HTTPAPI 공통 인증 로직
    middleware/a2a/     A2A 프로토콜 미들웨어
    middleware/httpapi/  HTTP API 미들웨어
    ...
    token/              (Phase 3 예정)
  ```
- [x] 모든 주석 영어. 파일/기능 단위로 핵심만 짧게
- [x] CI/CD: GitHub Actions (build, test, lint, vet)
- [x] `.goreleaser.yml` 설정

### 1.2 Transport Layer

- [x] **stdio 모드**: `shield-agent <command> [args...]`
  - 자식 프로세스 stdin/stdout 파이프 인터셉트
  - stderr 패스스루
  - SIGINT/SIGTERM 포워딩, exit code 전파
  - graceful shutdown (5초 → SIGKILL)
- [x] **proxy 모드**: `shield-agent proxy --listen :8888 --upstream <url>`
  - SSE transport (`GET /sse` + `POST /messages`)
  - Streamable HTTP transport (`POST /mcp`, `GET /mcp`, `DELETE /mcp`)
  - `--transport sse|streamable-http` 플래그
- [x] **TLS 지원**: `--tls-cert`, `--tls-key` 플래그
- [x] **CORS 설정**: YAML에서 allowed origins 제어
...
- [x] Middleware 인터페이스 설계
  ...
  ```
- [x] Chain: YAML 순서대로 로드, `enabled: false`면 스킵
- [x] CLI 오버라이드: `--disable-middleware auth` / `--enable-middleware token`
- [x] 런타임 reload: SIGHUP 시 YAML 재로드 (재시작 없이 적용)
...
- [x] `open` 모드 (경고만) / `closed` 모드 (거부)
- [x] KeyStore: FileKeyStore (YAML) + CachedKeyStore (TTL 5분)
- [x] Agent ID는 로그에 sha256 해시로만 기록

### 1.5 Guard Middleware (방어)

- [x] **Rate Limiting**
  - method별 분당 요청 제한 (fixed window)
  - 초과 시 JSON-RPC error 반환
- [x] **Request Size Limit**
  - body 크기 제한 (YAML로 설정 가능)
...
- [x] 요청/응답 쌍 SQLite 비동기 저장
  - 비동기 write channel (버퍼 512)
  - 채널 풀이면 drop + 경고 로그
  - Prometheus 카운터/히스토그램 연동
- [x] 스키마: timestamp, agent_hash, method, direction, success, latency_ms, payload_size, auth_status, error_code
- [x] 인덱스: timestamp, (agent_hash, timestamp), method
- [x] 자동 purge: retention_days(기본 30) 경과 항목 삭제
- [x] ip_address 컬럼 추가 (migration v2)

### 1.7 Storage (SQLite)

- [x] WAL 모드, busy timeout 5초
- [x] 테이블: `action_logs` (tokens, token_usage는 Phase 3)
- [x] 마이그레이션 시스템 (schema_versions 테이블, 버전 관리)
...
- [x] auth: 서명 검증, DID 해석, open/closed 모드
- [x] guard: rate limit 초과, size limit
- [x] middleware chain: 순서, 에러 전파
- [x] storage: CRUD, purge
- [x] transport: stdio 파이프라인
- [x] config: 우선순위, 검증
- [x] e2e: 통합 테스트
- [x] guard: IP 차단/허용, brute force, malformed JSON-RPC 테스트
- [x] transport: proxy 포워딩 테스트 (SSE, Streamable HTTP)
- [x] httpauth: 공통 인증 로직 단위 테스트
...
## Phase 5 — Egress 컴플라이언스 (AI 규제)

> 목표: 에이전트의 outbound 요청을 가로채 한국 AI기본법 / EU AI Act 감사 의무 충족

### 5.1 — Metadata-only forward proxy (완료)

- [x] `shield-agent egress --listen <addr>` 서브커맨드
- [x] `shield-agent proxy --with-egress`, `shield-agent <child> --with-egress` 공용 실행
- [x] CONNECT 터널링 + 평문 HTTP 프록시 (SSRF 가드, 공유 `http.Transport`)
- [x] `EgressMiddleware` + `EgressChain` + `SwappableEgressChain` (독립 파이프라인)
- [x] Provider 자동 감지 (openai / anthropic / google / cohere / mistral / azure)
- [x] Policy middleware (upstream allowlist, warn/block 모드)
- [x] Drop-free `LogWriter` (bounded-but-blocking channel, fail-closed EnqueueSync)
- [x] `action_logs.correlation_id` + `egress_logs` + `egress_log_anchors` (migration 10·11)
- [x] SHA-256 해시 체인 + purge 앵커 알고리즘 (`shield-agent logs --verify`)
- [x] 감사 번들 export (`shield-agent logs --export-audit`) — rows + anchors + proof + policy snapshot
- [x] Prometheus 메트릭 (requests/latency/bytes/policy_violations/log_write_errors/log_queue_length/hashchain_length)
- [x] Integration + MITM E2E + unit 테스트 세트, 벤치 하네스(`bench_test.go` + `scripts/bench-egress.sh`)

### 5.2 — TLS MITM + Deep inspection (완료, 법무 검토 후 활성화)

- [x] Root CA load-or-generate (P-256, 10년 기본 유효기간)
- [x] Leaf 인증서 minter + TTL 캐시 (기본 60분)
- [x] `shield-agent ca init` / `shield-agent ca trust` CLI (macOS/Linux 신뢰 저장소)
- [x] Per-host opt-in (`egress.mitm_hosts`), 그 외 호스트는 기존 CONNECT 터널 유지
- [x] 한국 특화 PII scrubber (주민번호, 전화, 이메일, 카드, 운전면허 + 커스텀 regex, mask/hash redaction)
- [x] Content tagger (OpenAI / Anthropic / Google model 추출 + fallback probe)
- [x] `compliance` 미들웨어 — prompt_hash + URL path PII scrub + response 분류 + X-AI-Generated 헤더 주입 옵션
- [x] 128-bit `correlation_id` 생성 + ingress context 전파
- [x] 외부 daily digest 파일 (`egress.hash_chain.digest_path`) — DB full-access 공격 방어
- [ ] 법무 검토 (TLS MITM 하의 PIPA 제23조 / GDPR Art. 9 리스크) — 외부 의존
- [ ] AI기본법 시행령/시행규칙 확정 시 export 라벨 갱신
- [ ] Phase 2 배포 전 대상 SDK의 certificate pinning 현황 최종 확인

### 5.3 — Follow-ups

- [ ] LLM 기반 PII 탐지 (regex 한계 보완)
- [ ] 외부 timestamp authority(RFC 3161)로 daily digest 서명
- [ ] gRPC-over-HTTP/2 egress 지원
- [ ] 다중 인스턴스 egress 로그 중앙 집계
- [ ] 원격 정책 서버 (기업 중앙 관리)
...
Phase 5 (Egress 컴플라이언스) ★★★★☆  — AI 규제 대응 (2026 시행 한국 AI기본법)
  ├── Phase 5.1 Metadata-only proxy — 완료
  ├── Phase 5.2 TLS MITM + PII scrub + content tagging — 완료 (법무 검토 후 활성화)
  └── Phase 5.3 follow-ups — LLM PII 탐지, RFC 3161 서명, gRPC, 중앙 집계
```
